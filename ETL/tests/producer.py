...
class Producer():
    ...
    def produce(self, set_num):
        ...
        if set_num <= 10:
            self.bpm(set_num)
        print(
            f"Test data set {set_num} produced in {int(time.time()) - start} seconds")
    ...
    def _validate_count(self, format, location, expected_count):
        ...
        print(
            f"Found {actual_count:,} / Expected {expected_count:,} records: Success")
    ...
    def validate(self, sets):
        ...
        self._validate_count(
            "csv", "registered_users_bz/1-registered_users", 5 if sets == 1 else 10)
        self._validate_count(
            "json", "kafka_multiplex_bz/2-user_info", 7 if sets == 1 else 13)
        self._validate_count("json", "kafka_multiplex_bz/3-bpm", sets * 253801)
        self._validate_count(
            "json", "kafka_multiplex_bz/4-workout", 16 if sets == 1 else 32)
        self._validate_count(
            "csv", "gym_logins_bz/5-gym_logins", 8 if sets == 1 else 16)
