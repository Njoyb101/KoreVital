...
class Gold():
    ...
    def upsert(self, once=True, processing_time="5 seconds"):
        ...
        print(
            f"Completed gold layer upsert {int(time.time()) - start} seconds")
    ...
    def assert_rows(self, location, table_name, sets):
        ...
        expected_rows = spark.read.format("parquet").load(
            f"{self.test_data_dir}/{location}_{sets}.parquet").collect()
        ...
        print(
            f"Expected data matches with the actual data in {table_name}: Success")
    ...
    def validate(self, sets):
        ...
        print(
            f"Gold layer validation completed in {int(time.time()) - start} seconds")
